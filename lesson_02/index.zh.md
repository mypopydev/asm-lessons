...
我们需要首先介绍标签（label）和跳转的概念。在下面的人工示例中，jmp 指令将代码执行移动到 ".loop:" 之后。".loop:" 被称为*标签*（label），标签前的点表示它是一个*局部标签*（local label），有效地允许你在多个函数中重用相同的标签名。当然，这个例子展示了一个无限循环，但我们稍后会将其扩展为更现实的内容。
